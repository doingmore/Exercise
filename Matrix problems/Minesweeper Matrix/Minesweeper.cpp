...
– either a . (dot), or a ! (exclamation mark) – representing "empty" or "mined" positions. Write a program that prints a N by M matrix,
where each cell contains a number, representing how many adjacent cells, including itself, are "mined". Each cell in a matrix
has at most 8 adjacent cells – the cells directly above, below, to the left, to the right, as those diagonally – to the left and above,
to the right and above, to the right and below and to the left and below.
*/
...
void findMines(const vector<string>& matrix,
    vector<int>& minesX,
    vector<int>& minesY)
{
    for (size_t  row = 0; row < matrix.size(); row++)
...
void expandSingleMine(vector<string>& mineField,
    const int ROW,
    const int COL)
{
...
void expandMines(vector<string>& mineField,
    const vector<int>& minesX,
    const vector<int>& minesY)
{
...
int main()
{
    ...
    vector<string> mineFieldValues(
    mineField.size(), string(mineField[0].size(), '0'));
